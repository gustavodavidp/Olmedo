...
namespace LetrasBlog.Client.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<CatFacts.response> GetCatsFacts()
        {
            CatFacts.response CatsData = new CatFacts.response();

            using HttpClient client = new();
            var cats = await ProcessRepositoriesAsync(client);

            CatsData = new CatFacts.response
            {
                Code = 0,
                Message = "Success",
                Books = new CatFacts.Data()
                {
                    fact = cats.Detail.fact,
                    length = cats.Detail.length
                }
        ...
        static async Task<CatFacts.response> ProcessRepositoriesAsync(HttpClient client)
        {
            await using Stream stream =
                await client.GetStreamAsync("https://catfact.ninja/facts");
            var repositories =
                await System.Text.Json.JsonSerializer.DeserializeAsync<CatFacts.response>(stream);
